...
pub struct IosurfaceSyntheticFrameSource {
    width: u32,
    height: u32,
...
pub struct IosurfaceSyntheticFrame {
    pub pixel_buffer: Nv12PixelBuffer,
    pub recycle_buffer: Nv12PixelBuffer,
    pub fill_elapsed: Duration,
}

impl IosurfaceSyntheticFrameSource {
    pub fn new(width: u32, height: u32, right_eye_shift_x_px: i32) -> Result<Self> {
    ...
    pub fn frame(&mut self, frame_index: u64) -> Result<Option<IosurfaceSyntheticFrame>> {
        ...
        buffer.fill_test_pattern(
            ...
            self.height,
            frame_index,
            self.right_eye_shift_x_px,
        )?;
        let pixel_buffer = buffer.clone();

        Ok(Some(IosurfaceSyntheticFrame {
            pixel_buffer,
